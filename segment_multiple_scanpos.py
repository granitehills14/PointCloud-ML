'''
Process Multiple Scan Positions in a project
'''
...
if __name__ == "__main__":
    ...
    # Initialize SAM
    device = "cuda" if torch.cuda.is_available() else "cpu" # if CUDA is available, use it, otherwise use the CPU
    MODEL = sam3["model"]
    ...
    ScanPos_folders = ld.set_up_scanpos_folders(project_dir)
    
    # loop through all scan positions in the project folder
    for scanpos_folder in ScanPos_folders: 
        SCANPOS = scanpos_folder.name
